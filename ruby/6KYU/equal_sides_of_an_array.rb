...
the index ({1}) and the sum of the right side of the index ({50,-51,1,1}) both 
equal 1.
Last one:
...
input = array 
output = integer that...
- is the index of the array where the sum of elements on the left side of the 
array are equal to the that on the right.

rule = 
- return the lowest correct index if input array has multiple answers
- if no index exist to balance the array, return -1

data strucutre = array 
algorithm = 
given an array...
- sum all elements of the array to get the total sum
- initialse a variable 'left_sum' to intger 0
- check if total - left_sum - index == right_sum...
- iterate over the input array with index 
- for each element passed into the block...
- check if the total sum - left_sum - element == leftsum(right_sum and left_sum 
are equal..so use left_sum as we have that figure available) 
  - if it is, return the index of the element
  - else update the count of left_sum by adding the element .. 
  left_sum += element and moving to the next element in the array
...
def find_even_index(array)
  ...
  array.each_with_index do |num, index|
    return index if total - left_sum - num == left_sum
    left_sum += num 
  end
  - 1
end
